...
**이 프로젝트는 포트폴리오용으로 의미있는 기능 단위로 커밋해주세요:**

#### 커밋 단위 원칙
1. **완전한 기능 단위**: 하나의 완전한 기능이 동작할 수 있는 상태로 커밋
2. **테스트 가능한 상태**: 빌드 오류나 컴파일 에러가 없는 상태로 커밋
3. **논리적 그룹핑**: 관련된 파일들을 함께 커밋 (예: DTO + Service + Controller)
4. **문서 동기화**: 코드 변경 시 관련 문서도 함께 업데이트하여 커밋
...
- Spring Security 설정 및 JWT 토큰 처리
- 사용자 로그인/회원가입 API 구현
- 토큰 갱신 및 인증 필터 추가
- 상세한 주석과 에러 처리 포함
